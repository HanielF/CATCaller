...
do
    ...
    # python /home/xudi/CATCaller/generate_dataset/inference_data.py -fast5 ${fast5_path} -records_dir ${tmp_records_dir} -raw_len ${signal_window_length} -threads 40
    ...
    python /home/xudi/CATCaller/caller.py -model ${model_path} -records_dir ${tmp_records_dir} -output ${basecalled_dir}/${p} -half
    endtime=`date +'%Y-%m-%d %H:%M:%S'`
    echo "${p} finished!"
    start_seconds=$(date --date="$starttime" +%s);
    end_seconds=$(date --date="$endtime" +%s);
    echo "${p} running time： "$((end_seconds-start_seconds))"s" > ${time_out_path}
    # echo "delete tmp records"
