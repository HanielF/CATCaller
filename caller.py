...
class Call(nn.Module):  #delete
    def __init__(self, opt):
        ...
        print("In Call, opt.device: {}".format(opt.device))
...
def writer(argv,item,write_mutex):
    ...
    # print("Before BeamCTCDecoder decode: probs: {} lengths: {} ".format(probs, lengths))
    # np.save("/home/xudi/CATCaller/probs.npy", probs)
    # np.save("/home/xudi/CATCaller/sizes.npy", lengths)
    decoded_output, offsets = decoder.decode(probs, lengths)
    # np.save("/home/xudi/CATCaller/decoded_output.npy", decoded_output)
    # np.save("/home/xudi/CATCaller/offsets.npy", offsets)

    a g c t  ag+gc+ct  agc gct motif

    if not os.path.exists("/home/xudi/CATCaller/catcaller_decode_dic.flag"):
        print("==> Start save")
        os.system(" touch /home/xudi/CATCaller/catcaller_decode_dic.flag ")
        res_dic = {"probs":probs, "lengths": lengths, "decoded_output": decoded_output, "offsets": offsets}
        # print(res_dic)
        torch.save(res_dic, "/home/xudi/CATCaller/catcaller_decode_dic", )
        print("==> Save done")
...
def decode_process(argv, qdecoder):
    ...
    while True:
        item = qdecoder.get(timeout=200) # (read_id_list, row_num_list, log_probs_list, output_lengths_list)
        try:
